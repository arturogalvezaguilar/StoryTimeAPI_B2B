...
@Injectable()
export class AppUserProgressionQuestionsByProductDAO extends DynamoDBDAO<
  AppUserProgressionQuestionsByProduct
> {
  async getItemFromDB(
    table: string,
    itemId: string
  ): Promise<AppUserProgressionQuestionsByProduct> {
    return super.getItemFromDB(table, itemId);
  }

  async getItemFromDBByKey(
    table: string,
    key: object
  ): Promise<AppUserProgressionQuestionsByProduct> {
  ...
  async putItemToDB(
    table: string,
    item: AppUserProgressionQuestionsByProduct
  ): Promise<CommonSuccessResponse | ErrorResponse> {
