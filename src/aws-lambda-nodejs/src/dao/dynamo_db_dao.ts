...
export class DynamoDBDAO<I> {
  ...
  public async getItemFromDBByKey(table: string, key: object): Promise<I> {
    ...
    try {
      ...
      if (!item) {
        console.log(`Item with id ${key} was not found inside ${table} table`);
        return null;
      }
      console.log("Item = ", JSON.stringify(item));
      return item;
  ...
  public async putItemToDB(
    ...
    try {
      ...
      console.log(
        `An error has occurred while putting item ${JSON.stringify(
          item
        )} to the ${table} table. ${e.toString()}`
      );
      return new CommonSuccessResponse(
