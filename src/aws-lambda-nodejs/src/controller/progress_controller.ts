...
export class ProgressController {
  constructor(
    private starsProgressService: StarProgressService,
  ...
  public async setStars(
    @Param("id") id: string,
    @Body() setStarsProgressRequest: SetStarsProgressRequest
  ) {
    console.log("Set stars progress API input = ", setStarsProgressRequest);
    const setStarsProgressResponse = await this.starsProgressService.setStarsProgress(
      id,
      setStarsProgressRequest
    );
  ...
  public async getGameProgress(
    @Param("id") userId: string,
    @Param("gameId") gameId
  ): Promise<Game> {
    console.log(
      `Getting game progress with userId = ${userId} and gameId = ${gameId} ...`
    );
    const game: Game = await this.pageProgressService.getGameProgress(
      userId,
      gameId
    );
  ...
  public async putGameProgress(
    @Param("id") userId: string,
    @Param("gameId") gameId,
    @Body() game: Game
  ) {
    console.log(
      `Putting game progress with userId = ${userId} and gameId = ${gameId} and game ${JSON.stringify(
        game
      )} ...`
    );
    const response = await this.pageProgressService.putGameProgress(
      userId,
      gameId,
      game
    );
  ...
  public async getVideoProgress(
    ...
    console.log(
      `Getting video progress with userId = ${userId} and videoId = ${videoId} ...`
    );
    const video: Video = await this.pageProgressService.getVideoProgress(
      userId,
      videoId
    );
  ...
  public async putVideoProgress(
    @Param("id") userId: string,
    @Param("videoId") videoId,
    @Body() video: Video
  ) {
    console.log(
      `Putting video progress with userId = ${userId} and videoId = ${videoId} and video ${JSON.stringify(
        video
      )} ...`
    );
    const response = await this.pageProgressService.putVideoProgress(
      userId,
      videoId,
      video
    );
  ...
  public async getCollectionProgress(
    @Param("id") userId: string,
    @Param("collectionId") collectionId
  ): Promise<Collection> {
    console.log(
      `Getting collection progress with userId = ${userId} and collectionId = ${collectionId} ...`
    );
    const collection: Collection = await this.pageProgressService.getCollectionProgress(
      userId,
      collectionId
    );
  ...
  public async putCollectionProgress(
    @Param("id") userId: string,
    @Param("collectionId") collectionId,
    @Body() collection: Collection
  ) {
    console.log(
      `Putting collection progress with userId = ${userId} and collectionId = ${collectionId} and video ${JSON.stringify(
        collection
      )} ...`
    );
    const response = await this.pageProgressService.putCollectionProgress(
      userId,
      collectionId,
      collection
    );
  ...
  public async putSpinnerProgress(
    @Body() input: SpinnerGameProgress
  ): Promise<CommonSuccessResponse> {
    console.log("Putting spinner progress " + JSON.stringify(input));
    const response:
      | SetUserDataResponse
      | ErrorResponse = await this.spinnerGameProgressService.setSpinnerGameProgress(
      input
    );
  ...
  public async getSpinnerProgress(
    @Body() input: GetSpinnerProgressRequest
  ): Promise<SpinnerGameProgress> {
    console.log("Getting spinner progress with input = ", input);
    const spinnerProgress:
      | SpinnerGameProgress
      | ErrorResponse = await this.spinnerGameProgressService.getSpinnerGameProgress(
      input
    );
