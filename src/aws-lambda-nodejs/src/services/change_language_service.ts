...
export class ChangeLanguageService {
  ...
  public async changeLanguage(
    ...
    const inputValidator: IValidator = validateInput(
      ["appUserID", "productID", "deviceID", "productIDNew"],
      input
    );
    ...
    if (!product) {
      return errorResponse(
        constants.NOT_FOUND.statusCode,
        constants.NOT_FOUND.status,
        `Item with ${input.productID} not found inside ${
          constants.PRODUCT_TABLE
        }`
      );
    }

    const productList: IProduct[] = this.getUpdatedProductsList(
      appUser.products,
      input.productIDNew,
      input.deviceID
    );
    ...
    if (vocabsData !== null) {
      await this.vocabsDAO.deleteItemFromBD(
        constants.APP_USER_PROGRESSION_BY_PRODUCT_TABLE,
        vocabsData.id
      );
    }
    ...
    if (videosData !== null) {
      await this.videosDAO.deleteItemFromBD(
        constants.APP_USER_PROGRESSION_VIDEOS_BY_PRODUCT_TABLE,
        videosData.id
      );
    }
    ...
    if (questionsData !== null) {
      await this.questionsDAO.deleteItemFromBD(
        constants.APP_USER_PROGRESSION_QUESTIONS_BY_PRODUCT_TABLE,
        questionsData.id
      );
    }
    ...
    return new ChangeLanguageResponse(
      appUser.id,
      appUser.deviceID,
      1,
      appUser.productID,
  ...
  private getUpdatedProductsList(
    existingProducts: IProduct[],
    newProductID: string,
    deviceID: string
  ): IProduct[] {
    const sameProduct = existingProducts.filter(
