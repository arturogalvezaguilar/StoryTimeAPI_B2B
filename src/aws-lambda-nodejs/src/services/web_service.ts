...
export class WebService {
  ...
  public async sendContactUsEmail(
    ...
    const isCaptchaTokenValid = await this.captchaValidator.validateReCAPTCHA(
      token
    );
    ...
    await this.mandrillClient.sendMail(
      constants.CONTACT_US_WEB_EMAIL_SUBJECT,
      template,
      email
    );
    const response: SESEmailSenderResponse = await this.sesEmailSender.sendTextEmail(
      ...
      composedMessage,
      constants.WEB_CONTACT_US_SUBJECT
    );
