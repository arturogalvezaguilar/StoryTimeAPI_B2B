...
export class AppUserService {
  ...
  public async getAppUser(
    ...
    const item = await this.appUserDAO.getItemFromDB(
  ...
  public async saveOptin(
    ...
    const inputValidator: IValidator = validateInput(
      ["appUserID", "deviceID", "productID", "isAdult"],
      input
    );
    ...
    if (input.isAdult === 0 && input.email == null) {
      return errorResponse(
        constants.BAD_REQUEST.statusCode,
        constants.BAD_REQUEST.status,
        "The body should contain email if isAdult = 0"
      );
    }
  ...
  public async signLegal(
    ...
    return new LegalSignedResponse(
      appUser.id,
      appUser.deviceID,
      1,
      appUser.productID,
  ...
  public async updateUserType(
    ...
    const inputValidator: IValidator = validateInput(
      ["appUserID", "deviceID", "productID", "userType"],
      input
    );
    ...
    if (!isInt(input.userType) || input.userType < 0 || input.userType > 6) {
      return errorResponse(
        constants.BAD_REQUEST.statusCode,
        constants.BAD_REQUEST.status,
        "UserType should be 0, 1, 2, 3, 4, 5 or 6"
      );
    }
  ...
  private async formResponse(item: IAppUser): Promise<GetAppUserByIdResponse> {
    const legal: Signed = {
      isSigned: item.isLegalSigned,
      date: item.legalDate
    };
    const ageOptIn: Signed = {
      isSigned: item.ageOptin,
      date: item.ageOptinDate
    };
    const signUpProcess: Signed = {
      isSigned: item.signUpProcess,
      date: item.signUpProcessDate
    };

    let subscription;

    try {
      subscription = await this.subscriptionHistoryDAO.getItemFromDB(
    ...
    const subscriptionPrice =
      subscription === null ? null : subscription.localPrice;
    const expireDate =
      subscription === null ? null : subscription.expirationDate;
