...
import {
  IVoucherValidatorAPIResponse,
  VoucherValidator
} from "../lib/voucher_validator";
...
export class LoginAlternativeService {
  ...
  public async loginAlternative(
    ...
    const appUser = await this.appUserDao.getItemFromDB(
    ...
    const operator = await this.operatorDao.getItemFromDBByKey(
      constants.OPERATOR_TABLE,
      { operatorName: input.operatorName.toLowerCase() }
    );
    const currentOperator = appUser.operatorsList.find(billingOperator => {
      return (
        billingOperator.operatorType.toLowerCase() ===
        input.operatorName.toLowerCase()
      );
    });
    ...
    if (!currentOperator) {
      return errorResponse(
        ...
        `Billing operator ${
          input.operatorName
        } not found for this user, ${JSON.stringify(appUser.operatorsList)}`
      );
    }
    ...
    if (
      appUser.signUpProcess === 0 &&
      appUser.userType === constants.ALTERNATIVE_GUEST
    ) {
      console.log("User is Alternative guest and not signed up");
      const appUserByMSISDN = await this.appUserDao.getItemByGSI(
        constants.APP_USER_TABLE,
        "MSISDN-index",
        "msisdn = :msisdn",
      ...
      if (appUserByMSISDN) {
        ...
        console.log(
          "Setting new deviceID " + input.deviceID + " to existing user"
        );
        console.log(
          "Setting registeredOperatorName " +
            input.operatorName +
            " to existing user"
        );
    ...
    if (
      isUserAlreadyExists &&
      currentOperator.operatorType === this.XL_OPERATOR
    ) {
      console.log(
        "Found user by MSISDN =  " +
          currentAppUser.msisdn +
          " with " +
          currentOperator.operatorType +
          " operator"
      );
      const dafaultDate = new Date(0);
      return new LoginAlternativeResponse(
        currentAppUser.id,
        currentAppUser.productID,
        currentAppUser.deviceID,
        currentAppUser.userType,
        currentAppUser.isLegalSigned,
        currentAppUser.legalDate,
        currentAppUser.ageOptin,
        currentAppUser.ageOptinDate,
        currentAppUser.signUpProcess,
        currentAppUser.signUpProcessDate,
        constants.NO_ERROR,
        1
      );
    }
    ...
    console.log(
      "Getting user state from MySQL with mySQLRequestBody = " +
        mySQLRequestBody
    );
    const mySQLResponseBody = await this.getUserStateFromMySQL(
      mySQLRequestBody
    );
    console.log("Response from MySQL = " + mySQLResponseBody);
    ...
    if (subscriptionState === -1) {
      if (isUserAlreadyExists) {
        ...
        await this.appUserDao.deleteItemFromBD(
          constants.APP_USER_TABLE,
          appUser.id
        );
      ...
      await this.appUserDao.putItemToDB(
        constants.APP_USER_TABLE,
        currentAppUser
      );
    ...
    if (nextModifiedDate) {
      ...
      if (
        !appUserProducts ||
        appUserProducts == null ||
        appUserProducts.length < 1
      ) {
      ...
      appUserProducts = this.fillUpProductList(
        input.productID,
        input.deviceID,
        lastModifiedDate,
        nextModifiedDate,
    ...
    if (
      subscriptionState === constants.ALTERNATIVE_CANCELLED ||
  ...
  public async validateVoucher(
    ...
    if (operatorName.toLowerCase() !== this.XL_OPERATOR) {
      return new ValidateVoucherResponse(
        constants.BAD_REQUEST.status,
        constants.BAD_REQUEST.statusCode,
        "Wrong operator name. Should be xl."
      );
    }

    const appUser: IAppUser = await this.appUserDao.getItemFromDB(
      constants.APP_USER_TABLE,
      appUserID
    );
    ...
    if (!appUser) {
      return new ValidateVoucherResponse(
        constants.NOT_FOUND.status,
        constants.NOT_FOUND.statusCode,
        `User with id ${appUserID} not found inside ${constants.APP_USER_TABLE}`
      );
    }

    const operator: IOperator = await this.operatorDao.getItemFromDBByKey(
      constants.OPERATOR_TABLE,
      { operatorName: input.operatorName.toLowerCase() }
    );
    ...
    if (!operator) {
      return new ValidateVoucherResponse(
        constants.NOT_FOUND.status,
        constants.NOT_FOUND.statusCode,
        `Operator with id ${operatorName} not found inside ${
          constants.OPERATOR_TABLE
        }`
      );
    }
    ...
    const validateVoucherAPIResponse: IVoucherValidatorAPIResponse = await this.voucherValidator.validateVoucher(
      msisdnWithCode,
      voucher,
      operatorName
    );

    const signUpProcess: ISignUpProcess = {
      isSigned: appUser.signUpProcess,
      date: appUser.signUpProcessDate
    };
    const ageOptIn: IAgeOptIn = {
    ...
    if (validateVoucherAPIResponse.status !== constants.OK.statusCode) {
      return new ValidateVoucherResponse(
        constants.OK.status,
        constants.OK.statusCode,
        `Voucher ${voucher} is not valid`,
        ageOptIn,
        appUser.id,
        appUser.deviceID,
        constants.VOUCHER_VALIDATION_FAILED,
        0,
        legal,
        appUser.productID,
        signUpProcess,
  ...
  protected fillUpProductList(
    ...
    const appUserProduct = new Product(
      productID,
      deviceID,
      subscriptionStart,
      subscriptionState,
  ...
  protected async updateInstallationInDB(
    existingInstallationId: string,
    newInstallationId: string
  ): Promise<void> {
    try {
      console.log("Updating installation information ...");
      const newInstallationInfo = await this.installationDAO.getItemFromDB(
        constants.INSTALLATION_TABLE,
        newInstallationId
      );
      let existingInstallation = await this.installationDAO.getItemFromDB(
        constants.INSTALLATION_TABLE,
        existingInstallationId
      );
      existingInstallation = this.updateInstallationInfo(
        existingInstallation,
        newInstallationInfo
      );
      console.log(
        "Deleting extra installation info  from DB" +
          newInstallationInfo.toString()
      );
      await this.installationDAO.deleteItemFromBD(
        constants.INSTALLATION_TABLE,
        newInstallationInfo.id
      );
      console.log(
        "Updating existing installation in DB " +
          existingInstallation.toString()
      );
  ...
  protected updateInstallationInfo(
    existingInstallation: IInstallation,
    newInstallationInfo: IInstallation
  ) {
    ...
    const historyInstallationInfo = new Installation(
      existingInstallation.appVersion,
      existingInstallation.country,
      existingInstallation.deviceAdvertisingID,
      existingInstallation.deviceManufacturer,
      existingInstallation.deviceModel,
      existingInstallation.id,
      existingInstallation.installationDate,
      existingInstallation.installationHistory,
      existingInstallation.language,
      existingInstallation.marketingReferral,
      existingInstallation.productID
    );
    ...
    existingInstallation.marketingReferral =
      newInstallationInfo.marketingReferral;
    existingInstallation.deviceManufacturer =
      newInstallationInfo.deviceManufacturer;
    ...
    existingInstallation.deviceAdvertisingID =
      newInstallationInfo.deviceAdvertisingID;
