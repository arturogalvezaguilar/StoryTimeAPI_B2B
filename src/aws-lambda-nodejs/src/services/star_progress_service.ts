...
export class StarProgressService {
  constructor(
    private starsProgressDAO: StarsProgressDAO,
    private appUserDAO: AppUserDAO
  ) {}

  public async getStarsProgress(appUserID: string): Promise<StarsProgress> {
    if (!appUserID || isEmpty(appUserID))
      throw new BadRequestException(
        "appUserID is empty or has wrong format",
    ...
    if (!starsProgress || starsProgress.length === 0)
      throw new NotFoundException(
        `Stars progress for id ${appUserID} not found`,
  ...
  public async setStarsProgress(
    appUserID: string,
    body: SetStarsProgressRequest
  ): Promise<StarsProgress> {
    if (!appUserID || isEmpty(appUserID))
      throw new BadRequestException(
        "appUserID is empty or has wrong format",
        constants.BAD_REQUEST.statusCode
    ...
    await this.starsProgressDAO.putItemToDB(
      constants.STARS_PROGRESS_TABLE,
      starsProgress
    );
