...
} from "../models/subscription_package_model";
import { ISubscriptionData } from "../models/subscription_data_model";
import {
  ISubscriptionHistory,
  SubscriptionHistory
} from "../models/subscription_history_model";
...
export class StoreService {
  ...
  public async sendIAPReceipt(
    ...
    const inputValidator: IValidator = validateInput(
      ["productID", "deviceID", "appUserID", "appsFlyerID", "package"],
      input
    );
    ...
    if (
      !storeID ||
      storeID < this.APP_STORE_ID ||
      storeID > this.GOOGLE_PLAY_ID
    ) {
    ...
    if (storeID === this.APP_STORE_ID) {
      ...
      const isReceiptValid = await this.appleValidator.isAppStoreReceiptValid(
        payload
      );
      ...
      const isAndroidReceiptValid = await this.googleValidator.isGooglePlayReceiptValid(
        packageName,
        subscriptionId,
        purchaseToken
      );
    ...
    console.log(
      `Generating current package ${JSON.stringify(subscriptionPackage)}`
    );

    const subscriptionData: ISubscriptionData =
      subscriptionPackage.subscriptionData;
    console.log(
      `Generating new Subscription Data ${JSON.stringify(subscriptionData)}`
    );

    let subscriptionHistory: ISubscriptionHistory = await this.subscriptionHistoryDAO.getItemFromDB(
      constants.SUBSCRIPTION_HISTORY_TABLE,
      appUserID
    );
    ...
    const purchaseDate = isEmpty(subscriptionPackage.purchaseDate)
      ? formatCurrentDate(new Date().getTime())
      : subscriptionPackage.purchaseDate;
    const expireDate = isEmpty(subscriptionData.expireDate)
      ? undefined
      : subscriptionData.expireDate;
    const remainingTime = isEmpty(subscriptionData.remainingTime)
      ? undefined
      : subscriptionData.remainingTime;
    const introductoryPricePeriod = isEmpty(
      subscriptionData.introductoryPricePeriod
    )
      ? undefined
      : subscriptionData.introductoryPricePeriod;
    const introductoryPrice = isEmpty(subscriptionData.introductoryPrice)
      ? undefined
      : subscriptionData.introductoryPrice;
    ...
    const subscriptions: ISubscriptionPackage[] = !subscriptionHistory.subscriptions
      ? []
    ...
    await this.subscriptionHistoryDAO.putItemToDB(
      constants.SUBSCRIPTION_HISTORY_TABLE,
      subscriptionHistory
    );
