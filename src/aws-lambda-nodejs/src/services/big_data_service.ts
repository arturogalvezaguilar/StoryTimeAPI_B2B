...
export class BigDataService {
  ...
  public async saveData(input: any): Promise<BigDataResponse | ErrorResponse> {
    ...
    if (queue === null) {
      ...
      if (queue === null) {
        return new BigDataResponse(
          false,
          `Could not create queue ${constants.QUEUE_NAME}`
        );
      }
    }
    ...
    if (!isMessageSent) {
      return new BigDataResponse(
        false,
        "Error has occurred while sending message"
      );
    }
  ...
  private createDataParseList(input: string): string[] {
    ...
    for (let i = 0; i < arr_1.length; i++) {
      if (arr_1[i + 1]) {
        const is_contains_time_on_page = arr_1[i + 1]
          .split(" ")[2]
          .includes("TimeOnPage");
        const time_on_page = is_contains_time_on_page
          ? " " + arr_1[i + 1].split(" ")[2]
          : "";
        let str = "";
