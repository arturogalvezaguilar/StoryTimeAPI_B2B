...
export class LoginRegularService {
  ...
  public async loginRegular(
    ...
    const inputValidator: IValidator = validateInput(
      ["productID", "deviceID", "appUserID", "registrationType", "email"],
      input
    );
    ...
    if (!appUser) {
      return errorResponse(
        ...
        `Item with ${input.appUserID} not found inside ${
          constants.APP_USER_TABLE
        }`
      );
    }
    ...
    if (
      ...
      if (!appUserByEmail) {
        ...
        return new LoginRegularResponse(
          appUser.id,
          appUser.deviceID,
          constants.EMAIL_DOES_NOT_EXIST_ERROR,
          0,
          appUser.productID,
          appUser.userType
        );
    ...
    if (+this.registrationType === constants.REGULAR) {
      ...
      if (!this.password) {
        ...
        return new LoginRegularResponse(
          ...
          constants.PASSWORD_IS_EMPTY_ERROR,
          0,
      ...
      failureReason = await this.checkEmail(
        this.email,
        this.registrationType,
        isUserAlreadyExists
      );
      if (failureReason === constants.NO_ERROR) {
        failureReason = await this.checkPassword(
          this.password,
          currentAppUser.password
        );
      }
      ...
      if (usersRegistrationType !== registrationType) {
        ...
        console.log(
          "Input regType = " +
            registrationType +
            " found regType = " +
            appUserByEmail.registrationType
        );
        return constants.USER_NOT_FOUND;
      }
    }
  ...
  private checkUserType(userType: number): number {
    ...
    if (
      userType === constants.IAP_GUEST ||
      userType === constants.ALTERNATIVE_GUEST
    ) {
    ...
    if (
      userType === constants.ALTERNATIVE_CANCELLED ||
      userType === constants.IAP_CANCELLED
    ) {
      console.log(
        "User type = " + userType + ". User's subscription is cancelled"
      );
    ...
    if (userType === constants.ALTERNATIVE_DISABLED) {
      console.log(
        "User type = " + userType + ". User's subscription is disabled"
      );
      return constants.DISABLED_USER;
    }

    if (
      userType === constants.ALTERNATIVE_SUBSCRIBED ||
      userType === constants.IAP_SUBSCRIBED
    ) {
      console.log(
        "User type = " + userType + ". User's subscription is active"
      );
      return constants.NO_ERROR;
    }
    return constants.NO_ERROR;
  ...
  private async updateInstallationInfo(
    existingInstallationID: string,
    newInstallationID: string
  ) {
    console.log(`Updating installation info ... `);

    let existingInstallation: IInstallation = await this.installationDAO.getItemFromDB(
      constants.INSTALLATION_TABLE,
      existingInstallationID
    );
    const newInstallation: IInstallation = await this.installationDAO.getItemFromDB(
      constants.INSTALLATION_TABLE,
      newInstallationID
    );

    const existingInstallationHistory = !existingInstallation.installationHistory
      ? []
