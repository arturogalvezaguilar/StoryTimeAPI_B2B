...
export class LogoutService {
  constructor(private appUserDAO: AppUserDAO, private productDAO: ProductDAO) {}
  ...
  public async logout(input: any): Promise<LogoutResponse | ErrorResponse> {
    ...
    const product = await this.productDAO.getItemFromDB(
    ...
    if (appUser.productID !== input.productID) {
      return errorResponse(
        constants.BAD_REQUEST.statusCode,
        constants.BAD_REQUEST.status,
        "Invalid product for user supplied"
      );
    }
