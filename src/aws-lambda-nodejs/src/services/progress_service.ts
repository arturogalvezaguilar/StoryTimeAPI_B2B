...
export class ProgressService {
  constructor(
    private appUserProductProgressionDAO: AppUserProgressionDAO,
  ...
  public async updateQuestionsProgress(
    appUser: IAppUser,
    appUserID: string,
    questionsProgress: Array<AppUserProgressionQuestions>
  ): Promise<void> {
    ...
    questionsUserDataList =
      progress.questionsUserDataList === (null || void 0)
        ? []
        : progress.questionsUserDataList;
    progress.questionsUserDataList = [
      ...questionsUserDataList,
      ...questionsProgress
    ];
    console.log(`Putting progress levels to DB: ${JSON.stringify(progress)}`);
    await this.appUserProductProgressionDAO.putItemToDB(
      constants.PROGRESS_TABLE,
      progress
    );
  }

  public async updateLevelsProgress(
    appUser: IProgress,
    levelsProgress: Array<AppUserProgressionLevels>
  ) {
    let progress: IProgress;
    ...
    levelsUserDataList =
      progress.levelsUserDataList === (null || void 0)
        ? []
        : progress.levelsUserDataList;
  ...
  public async updateVocabsProgress(
    appUser: IProgress,
    vocabsProgress: Array<IAppUserProgressionVocabs>
  ) {
    let progress: IProgress;
    ...
    vocabsUserDataList =
      progress.vocabsUserDataList === (null || void 0)
        ? []
        : progress.vocabsUserDataList;
  ...
  public async updateVideosFranchiseProgress(
    appUser: IProgress,
    videoFranchiseProgress: Array<IAppUserProgressionVideosFranchise>
  ) {
    let progress: IProgress;
    ...
    videoFranchiseDataList =
      progress.videosFranchiseUserDataList === (null || void 0)
        ? []
        : progress.videosFranchiseUserDataList;
    progress.videosFranchiseUserDataList = [
      ...videoFranchiseDataList,
      ...videoFranchiseProgress
    ];
    console.log(
      `Putting progress franchise to DB: ${JSON.stringify(progress)}`
    );
    await this.appUserProductProgressionDAO.putItemToDB(
      constants.PROGRESS_TABLE,
      progress
  ...
  public async updateVideosProgress(
    appUser: IProgress,
    videoProgress: Array<AppUserProgressionVideos>
  ) {
    let progress: IProgress;
    ...
    videoDataList =
      progress.videoUserDataList === (null || void 0)
        ? []
        : progress.videoUserDataList;
  ...
  public async saveProgressionQuestions(
    ...
    const inputValidator: IValidator = validateInput(
      ["appUserID", "productID", "versionID", "questionsUserDataList"],
      input
    );
    ...
    const appUserInput: IAppUser = await this.appUserDAO.getItemFromDB(
    ...
    const userProgressions: Array<
      AppUserProgressionQuestionsByProduct
    > = await this.AppUserProgressionQuestionsByProductDAO.getItemsByGSI(
    ...
    if (userProgressions === null) {
      userProgression = new AppUserProgressionQuestionsByProduct(
        userID,
        currentDate,
        currentDate,
        productID,
        input.questionsUserDataList,
        input.versionID
      );
      await this.AppUserProgressionQuestionsByProductDAO.putItemToDB(
        constants.APP_USER_PROGRESSION_QUESTIONS_BY_PRODUCT_TABLE,
        userProgression
      );
      await this.updateQuestionsProgress(
        appUserInput,
        userID,
        input.questionsUserDataList
      );
  ...
  public async getProgressionQuestions(
    ...
    if (userProgressions === null) {
      const currentDate = formatCurrentDate(new Date().getTime());
      return new AppUserProgressionQuestionsByProduct(
        userID,
        currentDate,
        currentDate,
        productID,
