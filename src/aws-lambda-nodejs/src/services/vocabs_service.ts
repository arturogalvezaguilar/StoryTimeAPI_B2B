...
export class VocabsService {
  ...
  public async saveAppUserProgressionVocabs(
    ...
    const userProgression: AppUserProductProgression = await this.appUserProductProgressionDAO.getItemByGSI(
  ...
  private async updateUsersProgression(appUser, input): Promise<void> {
    ...
    try {
      ...
      await this.progressService.updateLevelsProgress(
        appUser,
        appUser.lastPlayedLevel
      );

      await this.progressService.updateVocabsProgress(
        appUser,
        appUser.vocabsUserDataList
      );
    } catch (error) {
