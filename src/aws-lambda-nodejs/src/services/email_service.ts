...
import {
  IVerificationToken,
  VerificationToken
} from "../models/verification_token_model";
...
export class EmailService {
  ...
  public async validateEmail(
    ...
    if (!verificationTokenModel) {
      return errorResponse(
        ...
        `Item with ${token} id not found inside ${
          constants.VERIFICATION_TOKEN_TABLE
        }`
      );
    }
    ...
    if (!verificationTokenModel.active) {
      return new ValidateEmailResponse(
        constants.TOKEN_NOT_ACTIVE,
        `This email is not active anymore!`
      );
    }

    const currentTimeMs = new Date().getTime();
    const tokenExpirationDateMs = new Date(
      verificationTokenModel.validUntilDate
    ).getTime();
    ...
    if (currentTimeMs > tokenExpirationDateMs) {
      ...
      await this.verificationTokenDAO.putItemToDB(
        constants.VERIFICATION_TOKEN_TABLE,
  ...
  public async sendWelcomeEmail(
    ...
    const productValidator: IValidator = validateProductIDFormat(
      input.productID
    );
    ...
    if (!appUser) {
      return errorResponse(
        constants.NOT_FOUND.statusCode,
        constants.NOT_FOUND.status,
        `AppUser with ${appUserID} id was not found inside ${
          constants.APP_USER_TABLE
        }`
      );
    }
  ...
  public async validatePassword(
    ...
    if (!appUser) {
      return errorResponse(
        constants.NOT_FOUND.statusCode,
        constants.NOT_FOUND.status,
        `AppUser with ${encodedEmail} was not found inside ${
          constants.APP_USER_TABLE
        }`
      );
    }
  ...
  public async resendVerificationEmail(
    ...
    if (!verificationTokenModels) {
      return errorResponse(
        constants.NOT_FOUND.statusCode,
        constants.NOT_FOUND.status,
        `Verification token with ${appUser.email} was not found inside ${
          constants.VERIFICATION_TOKEN_TABLE
        }`
      );
    }

    const activeToken = verificationTokenModels.filter(
      token => token.active
    )[0];
  ...
  public async resetPassword(
    ...
    if (!appUser) {
      return errorResponse(
        constants.NOT_FOUND.statusCode,
        constants.NOT_FOUND.status,
        `AppUser with ${encodedEmail} email was not found inside ${
          constants.APP_USER_TABLE
        }`
      );
    }
  ...
  public async sendVerificationEmail(
    ...
    const verificationEmailTemplate =
      constants.VALIDATION_TEMPLATE_B2B + language.toUpperCase();
    const token: IVerificationToken = this.createVerificationToken(
      appUserID,
      language,
      encodedEmail
    );
    const redirectLink: string = `${encodeURIComponent(
      token.id
    )}&m=${encodeURIComponent(encodedEmail)}`;
  ...
  public async sendResetPasswordEmail(
    ...
    const resetPasswordTemplate =
      constants.RESET_PASSWORD_TEMPLATE_B2B + language.toUpperCase();
    const token: IVerificationToken = this.createVerificationToken(
      appUserID,
      language,
      encodedEmail,
  ...
  public async isEmailVerified(
    input: any
  ): Promise<IIsEmailVerifiedResponse | ErrorResponse> {
    const inputValidator: IValidator = validateInput(
      ["appUserID", "deviceID", "productID"],
      input
    );
    ...
    if (!appUser) {
      return errorResponse(
        constants.NOT_FOUND.statusCode,
        constants.NOT_FOUND.status,
        `AppUser with ${appUserID} was not found inside ${
          constants.APP_USER_TABLE
        }`
      );
    }
  ...
  public async sendContactUsEmail(
    ...
    const subject = productID + ": " + appUserID + "(" + appVersion + ")";

    const response: SESEmailSenderResponse = await this.sesEmailSender.sendTextEmail(
      constants.CONTACT_US_SENDER_EMAIL,
      constants.CONTACT_US_RECIPIENT_EMAIL,
      message,
      subject
    );
  ...
  private createVerificationToken(
    ...
    console.log(
      `Creating verification token ${JSON.stringify(verificationToken)}`
    );
