...
export class LaunchAppService {
  ...
  public async launchApp(
    ...
    const fullProductID =
      input.productID + "-" + lookUpCountry.productLanguage.toLowerCase();
    ...
    const operatorsList: IProvider[] =
      lookUpBilling === null ? [] : lookUpBilling.provider;
    const productsList: IProduct[] = this.getNewProductsList(
      input.deviceID,
      productName
    );
  ...
  private async defineContentVersion(clientVersion): Promise<string> {
    ...
    const fetchedClientVersion: ClientVersion = await this.clientVersionDAO.getItemFromDB(
      constants.CLIENT_VERSION_TABLE,
      clientVersion
    );

    if (!fetchedClientVersion) {
      console.log(
        `Client version ${clientVersion} not found inside ${
          constants.CLIENT_VERSION_TABLE
        }`
      );
      console.log(
        "Setting up initial content ",
        constants.CONTENT_INITIAL_VERSION
      );
