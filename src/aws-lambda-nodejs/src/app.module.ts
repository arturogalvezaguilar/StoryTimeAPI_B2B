...
export class AppModule {
  public configure(consumer: MiddlewareConsumer) {
    consumer
      .apply(requestValidatorMiddleware(launchApplicationSchema))
      .forRoutes(constants.LAUNCH_APPLICATION_PATH);
    consumer
      .apply(requestValidatorMiddleware(legalSignedSchema))
      .forRoutes(constants.LEGAL_SIGNED_PATH);
    consumer
      .apply(requestValidatorMiddleware(loginAlternative))
      .forRoutes(constants.LOGIN_ALTERNATIVE_PATH);
    consumer
      .apply(requestValidatorMiddleware(loginRegular))
      .forRoutes(constants.LOGIN_REGULAR_PATH);
    consumer
      .apply(requestValidatorMiddleware(logoutSchema))
      .forRoutes(constants.LOGOUT_PATH);
    consumer
      .apply(requestValidatorMiddleware(singUpSchema))
      .forRoutes(constants.SIGN_UP_PATH);
    consumer
      .apply(requestValidatorMiddleware(validateEmailSchema))
      .forRoutes(constants.VALIDATE_EMAIL_PATH);
    consumer
      .apply(requestValidatorMiddleware(validateVoucher))
      .forRoutes(constants.VALIDATE_VOUCHER_PATH);
  }
}
