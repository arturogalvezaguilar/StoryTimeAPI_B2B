...
export class GoogleReceiptValidator {
  ...
  public async isGooglePlayReceiptValid(
    inputPackageName: string,
    inputSubscriptionId: string,
    purchaseToken: string
  ): Promise<boolean> {
    console.log(
      `Getting receipt info from Google Play for id ${inputSubscriptionId} and payload ${purchaseToken}`
    );
    ...
    try {
      ...
      if (!receipt.orderId) {
        console.log(
          `Subscription ${JSON.stringify(receipt)} was not found in GooglePlay`
        );
        return false;
      }
      ...
      console.log(
        `An error has occurred while getting response from Google Play ${e.toString()}`
      );
      return false;
    }
  }
