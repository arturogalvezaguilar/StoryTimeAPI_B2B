...
export const encodeMD5 = async (input: string) => {
  ...
  try {
    console.log(
      `Getting md5 encoded password  with parameters ${JSON.stringify(params)}`
    );
    const response = await lambda.invoke(params).promise();
    const password = response.Payload.toString()
      .replace('"', "")
      .replace('"', "");
    ...
    console.log(
      `An error has occurred while getting MD5 encoded password with params ${JSON.stringify(
        params
      )}`
    );
