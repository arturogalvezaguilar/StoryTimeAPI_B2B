...
export class VoucherValidator {
  private readonly VALIDATE_VOUCHER_URL =
    "https://222pdwf62i.execute-api.us-east-1.amazonaws.com/PRD/v1/partner-management/triyacom/bundle/subscriptions";
  ...
  public async validateVoucher(
    ...
    try {
      const response = await fetch(this.VALIDATE_VOUCHER_URL, {
        method: "POST",
        body: JSON.stringify(body)
      });
      result.body = await response.json();
      result.status = await response.status;
    } catch (e) {
      console.error(e);
