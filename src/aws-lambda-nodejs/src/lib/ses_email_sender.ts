...
export class SESEmailSender {
  ...
  public async sendTextEmail(
    ...
    textMessage: string,
    subject: string
  ): Promise<SESEmailSenderResponse> {
    ...
    console.log(
      `SES sending email with request `,
      JSON.stringify(sendEmailRequest)
    );
    try {
      const messageID = await ses.sendEmail(sendEmailRequest).promise();
      response = {
        isEmailSent: true,
        status: constants.OK.status,
        statusCode: constants.OK.statusCode
      };
      return response;
    } catch (e) {
